...
* [Четвертая A лаба](#четвертая-a-лаба-вариант-1)
...
## Четвертая A лаба (вариант 1)

Лаба находится в папке lab4a

### Автор
* Кабанов Андрей

### Предисловие
Лаба сделана мной чисто по приколу. Влад сам сделает часть лабы, которую я вырежу. И мы сдадим ее вместе.
### Запуск
```shell
julia main.jl <1>
```
Аргумент это путь к файлу с входными данными
#### Формат файла с входными данными
```
#priority \\ falst сначала по старше, true по младше
false
#string \\ слово оно если что одно, просто многострочное
(k+)+k*k
(k+k)+*k

#grammer \\ грамматика через пробелы, слитно ничего не писать
S ➡ E $ E $
E ➡ E + T | T
T ➡ T * F | F
F ➡ ( E ) | k
